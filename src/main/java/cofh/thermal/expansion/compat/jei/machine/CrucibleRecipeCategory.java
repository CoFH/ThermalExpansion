...
public class CrucibleRecipeCategory extends ThermalRecipeCategory<RecipeHolder<CrucibleRecipe>> {

    protected IDrawableStatic tankBackground;
    protected IDrawableStatic tankOverlay;

    public CrucibleRecipeCategory(IGuiHelper guiHelper, ItemStack icon, RecipeType<RecipeHolder<CrucibleRecipe>> type) {
    ...
    public void draw(RecipeHolder<CrucibleRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        if (!recipe.value().getOutputFluids().isEmpty()) {
            RenderHelper.drawFluid(guiGraphics, 74, 24, recipe.value().getOutputFluids().get(0), 24, 16);
