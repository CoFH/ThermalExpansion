...
public class BrewerRecipeCategory extends ThermalRecipeCategory<RecipeHolder<BrewerRecipe>> {
    ...
    public BrewerRecipeCategory(IGuiHelper guiHelper, ItemStack icon, RecipeType<RecipeHolder<BrewerRecipe>> type) {
    ...
    @Override
    public RecipeType<RecipeHolder<BrewerRecipe>> getRecipeType() {

        return type;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<BrewerRecipe> recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 116, 11)
                .addIngredients(NeoForgeTypes.FLUID_STACK, List.of(outputFluids.get(0)))
                .setFluidRenderer(tankSize(TANK_MEDIUM), false, 16, 40)
    ...
    @Override
    public void draw(RecipeHolder<BrewerRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
