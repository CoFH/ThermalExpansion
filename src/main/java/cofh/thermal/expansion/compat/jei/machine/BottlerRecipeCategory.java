...
import static cofh.thermal.lib.util.ThermalIDs.ID_MACHINE_BOTTLER;

public class BottlerRecipeCategory extends ThermalRecipeCategory<RecipeHolder<BottlerRecipe>> {

    protected IDrawableStatic tankBackground;
    protected IDrawableStatic tankOverlay;

    public BottlerRecipeCategory(IGuiHelper guiHelper, ItemStack icon, RecipeType<RecipeHolder<BottlerRecipe>> type) {
    ...
    @Override
    public RecipeType<RecipeHolder<BottlerRecipe>> getRecipeType() {

        return type;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<BottlerRecipe> recipe, IFocusGroup focuses) {
    ...
    @Override
    public void draw(RecipeHolder<BottlerRecipe> recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        if (!recipe.value().getInputFluids().isEmpty()) {
            RenderHelper.drawFluid(guiGraphics, 78, 24, recipe.value().getInputFluids().get(0).getFluids()[0], 24, 16);
